...
def investimentos(request):
    dados = {
        'dados': Investimeto.objects.all()
    }
    return render(request, 'investimento/investimentos.html', context=dados)

def detalhe(request, id_investimento):
    dados = {
        'dados': Investimeto.objects.get(pk=id_investimento)
    }
...
def editar(request, id_investimento):
    investimento = Investimeto.objects.get(pk=id_investimento)
...
def excluir(request, id_investimento):
    ...
    if request.method == 'POST':
        investimento.delete()
        return redirect('investimentos')
    return render(request, 'investimento/exclusao.html', {'item': investimento})
